...
class TestCommentAdd:
    ...
    def test_comment_add_success(self, authenticated_client, snippet, comment_data):
        ...
        response = client.post(reverse('MainApp:comment_add'), comment_data)

        # Проверяем, что произошло перенаправление на страницу сниппета
        assert response.status_code == 302
        assert response.url == reverse('MainApp:snippet-detail', kwargs={'id': snippet.id})
    ...
    def test_comment_add_invalid_form_data(self, authenticated_client, snippet):
        ...
        # Отправляем POST запрос
        response = client.post(reverse('MainApp:comment_add'), invalid_data)
    ...
    def test_comment_add_get_request(self, client):
        """Тест GET запроса к comment_add (должен вернуть редирект на логин)"""
        response = client.get(reverse('MainApp:comment_add'))
        assert response.status_code == 302
        assert '/login' in response.url or '/custom_login' in response.url
    ...
    def test_comment_add_missing_snippet_id(self, authenticated_client):
        ...
        data = {
            'text': 'Комментарий без snippet_id'

        }
        response = client.post(reverse('MainApp:comment_add'), data)


        assert response.status_code in (404, 400)

        assert not Comment.objects.filter(text='Комментарий без snippet_id').exists()
